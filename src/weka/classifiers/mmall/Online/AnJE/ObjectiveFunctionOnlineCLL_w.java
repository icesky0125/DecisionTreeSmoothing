...
public class ObjectiveFunctionOnlineCLL_w extends ObjectiveFunctionOnlineCLL {

	public ObjectiveFunctionOnlineCLL_w(wdAnJEOnline algorithm) {
		super(algorithm);		
	}
...
//		}
		...
		for (int c = 0; c < nc; c++) {			
			if (m_Regularization) {
				dParameters.setGradientAtFullIndex(c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c) + m_Lambda * dParameters.getParameterAtFullIndex(c));
			} else {
				dParameters.setGradientAtFullIndex(c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c));
			}
		}
		...
		if (m_S.equalsIgnoreCase("A1JE")) {
			...
			for (int c = 0; c < nc; c++) {
				for (int att1 = 0; att1 < n; att1++) {
					...
					if (m_Regularization) {
						dParameters.setGradientAtFullIndex(index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index));
					} else {
						dParameters.setGradientAtFullIndex(index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
					}
				}
			}
