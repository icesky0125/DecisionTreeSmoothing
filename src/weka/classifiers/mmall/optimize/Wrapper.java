package weka.classifiers.mmall.optimize;
...
public class Wrapper{
	...
	Wrapper(int variablesNo, int correctionsNo) {
		this.data = lbfgsb_wrapper.lbfgsb_create(variablesNo, correctionsNo);
	}
	...
	public IterationsInfo iterate(DifferentiableFunction fun, int maxIterations, IterationFinishedListener listener) throws LBFGSBException {
		...
		while (true) {
			...
			if (maxIterations>0 && iterationsFinished==maxIterations) {
				stopAlgorithm();
				return new IterationsInfo(iterationsFinished, functionEvaluations, IterationsInfo.StopType.MAX_ITERATIONS, getStateDescription());
			}
			...
			case LBFGSB_FG:
				...
				if (vals == null) {
					stopAlgorithm();
					return new IterationsInfo(iterationsFinished, functionEvaluations, IterationsInfo.StopType.USER, getStateDescription());
				}
			...
			case LBFGSB_NEW_X:
				iterationsFinished++;
				if (listener != null) {
					boolean continueAlg = listener.iterationFinished(getX(),	getFunctionValue(), getGradient());
			...
			case LBFGSB_CONV:
				return new IterationsInfo(iterationsFinished, functionEvaluations, IterationsInfo.StopType.OTHER_STOP_CONDITIONS, getStateDescription());
			
			case LBFGSB_ABNO:
				return new IterationsInfo(iterationsFinished, functionEvaluations, IterationsInfo.StopType.ABNORMAL, getStateDescription());
