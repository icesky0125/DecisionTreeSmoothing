				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {
						...
						if (m_Regularization) {
							...
							dParameters.incGradientAtFullIndex(g, index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index2));
						} else {							
							dParameters.incGradientAtFullIndex(g, index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
						}						
					}
				}
