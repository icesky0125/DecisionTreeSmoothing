...
public class CallableMSE_w implements Callable<Double>{
	...
	public CallableMSE_w(Instances instances, int start, int stop, int nc, int n, String m_S, double[] myProbs, double[] g, wdAnJEParameters dParameters) {
	...
	public CallableMSE_w(int start, int stop, double[] myProbs, double[] g, wdAnJE algorithm) {
	...
	public Double call() throws Exception {
		...
		for (int i = start; i <= stop; i++) {
			...
			for (int c = 0; c < nc; c++) {
				for (int k = 0; k < nc; k++) {
//					g[c] += (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c] * dParameters.getXyCount()[c];
					dParameters.incGradientAtFullIndex(g, c, (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c] * dParameters.getParameterAtFullIndex(c));
				}
			}
			...
			if (m_S.equalsIgnoreCase("A1JE")) {
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {
						...
						for (int k = 0; k < nc; k++) {
//							g[index] += (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c] * dParameters.getXyCount()[index];
							dParameters.incGradientAtFullIndex(g, index, (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c] * dParameters.getProbAtFullIndex(index));
						}
					}
				}
