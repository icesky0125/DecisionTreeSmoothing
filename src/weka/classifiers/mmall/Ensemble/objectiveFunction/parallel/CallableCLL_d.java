...
public class CallableCLL_d implements Callable<Double> {
	...
	public CallableCLL_d(Instances instances, int start, int stop, int nc, int n, String m_S, double[] myProbs, double[] g, wdAnJEParameters dParameters) {
	...
	public CallableCLL_d(int start, int stop, double[] myProbs, double[] g, wdAnJE algorithm, int th) {
		...
		for (int i = start; i <= stop; i++) {
			...
			for (int c = 0; c < nc; c++) {
				...
				if (m_Regularization) {
					...
					dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) + m_Lambda * dParameters.getParameterAtFullIndex(c));
				} else {
					dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
				}				
			}
				...
				for (int att1 = 0; att1 < n; att1++) {
					...
					for (int c = 0; c < nc; c++) {
						...
						if (m_Regularization) {
							...
							dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) + m_Lambda * dParameters.getParameterAtFullIndex(index));
						} else {
							dParameters.incGradientAtFullIndex(g, index,  (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
						}	
					}
