...
public class CallableCLL_wf implements Callable<Double>{
	...
	public CallableCLL_wf(Instances instances, int start, int stop, int nc, int n, String m_S, double[] myProbs, double[] g, wdAnJEParameters dParameters) {
	...
	public CallableCLL_wf(int start, int stop, double[] myProbs, double[] g, wdAnJE algorithm, int th) {
...
//				g[c] += (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c);
...
//						g[index] += (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index);
