		...
		for (int i = start; i <= stop; i++) {
			...
			for (int c = 0; c < nc; c++) {
				...
				if (m_Regularization) {
					...
					//dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c) + m_Lambda * dParameters.getParameterAtFullIndex(c) * dParameters.getProbAtFullIndex(c));
				} else {
					dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c));
				}
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {
						...
						if (m_Regularization) {
							...
							//dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index) * dParameters.getProbAtFullIndex(index));
						} else {							
							dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
						}
					}
