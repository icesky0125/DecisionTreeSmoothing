...
public class ObjectiveFunctionCLL_w extends ObjectiveFunction {
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int c = 0; c < nc; c++) {
				if (m_Regularization) {
					...
					//negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(c) * dParameters.getParameterAtFullIndex(c) * dParameters.getProbAtFullIndex(c) * dParameters.getProbAtFullIndex(c);
					//dParameters.incGradientAtFullIndex(c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c) + m_Lambda * dParameters.getParameterAtFullIndex(c) * dParameters.getProbAtFullIndex(c));
				} else {
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {
						...
						if (m_Regularization) {
							...
							//negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(index) * dParameters.getParameterAtFullIndex(index) * dParameters.getProbAtFullIndex(index) * dParameters.getProbAtFullIndex(index);
							//dParameters.incGradientAtFullIndex(index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index) * dParameters.getProbAtFullIndex(index));
						} else {							
		...
		//System.out.println();
		//System.out.println(Arrays.toString(dParameters.getGradients()));
		//System.out.println();
