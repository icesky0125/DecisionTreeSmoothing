...
public class A5JELogDistributionComputerMAP2 extends LogDistributionComputerAnJE{

	public static LogDistributionComputerAnJE singleton = null;
	
	protected A5JELogDistributionComputerMAP2(){}
	public static LogDistributionComputerAnJE getComputer() {
		if(singleton==null){
			singleton = new A5JELogDistributionComputerMAP2();
		}
		return singleton;
	}
	...
	public void compute(double[] probs, wdAnJEParameters params,Instance inst) {
		...
		for (int c = 0; c < probs.length; c++) {
			probs[c] -=  ((SUtils.NC5(params.getNAttributes()) - 1) * params.getProbAtFullIndex(c));
		}
	}
